...
for(i in 2:8){
  ...
  print(paste0(i, " Dice - Sword: ", sword(rolls)/10000, " Dagger: ", dagger(rolls)/10000))
}
```
...
for(i in 2:9){
  rolls = as.matrix(replicate(10000, sample(1:6, size = i, replace = TRUE)))
  print(paste0(i, " Dice - Sword: ", sword_plus(rolls)/10000, " Dagger: ", dagger_plus(rolls)/10000))
}
```
...
for(i in 2:9){
  ...
  print(paste0(i, " Dice - Sword: ", sword(frozen_rolls)/10000, " Dagger: ", dagger(frozen_rolls)/10000))
}
```

Predictably, it does not take long for the dagger to eclipse the sword in terms of expected damage. When using at least three dice, the dagger will always be expected to deal more damage than the sword. In fact, if a player is only using the dagger, having the "Frozen 1" status will result in a higher expected damage value than not having this status at all.
...
for(i in 2:9){
  rolls = as.matrix(replicate(10000, sample(1:6, size = i, replace = TRUE)))
  frozen_rolls <- freeze(rolls)
  print(paste0(i, " Dice - Sword: ", sword_plus(frozen_rolls)/10000, " Dagger: ", dagger_plus(frozen_rolls)/10000))
}
```

Moreover, in the event that the player is using the upgraded version of these weapons, the dagger will always have a higher damage output than the sword when "Frozen 1" is applied!